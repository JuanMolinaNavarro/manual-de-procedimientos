...
/**
 * Recibos de sueldo cerrados de una persona (módulo Nómina). Vive en el modal
 * y no en la pestaña porque el resultado decide si la pestaña se muestra.
 */
...
export default function RecibosDelEmpleado({ empleadoId, recibos, error }: EstadoRecibosEmpleado & { empleadoId: number }) {
  if (error) return <p className="text-sm text-destructive">{error}</p>;
  if (recibos === null) return <p className="text-sm text-muted-foreground">Cargando recibos…</p>;
  if (recibos.length === 0) return <p className="text-sm text-muted-foreground">Sin recibos cerrados todavía.</p>;

  return (
    <div className="max-h-[350px] space-y-2 overflow-y-auto pr-1.5">
      {recibos.map((r) => (
        <div key={r.liquidacionId} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3">
          <div className="min-w-0">
            <a
              href={`/admin/nomina/recibos/imprimir?organigramaId=${r.organigramaId}&periodo=${r.periodo}&empleadoId=${empleadoId}`}
              target="_blank"
              rel="noopener"
            ...
            <p className="font-mono text-[11px] text-muted-foreground">{r.organigramaNombre} · cerrado {fechaHora(r.fechaCierre)}</p>
          </div>
