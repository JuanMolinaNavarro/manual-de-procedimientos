...
import type { ContratoVendedor } from '@/lib/senales-ip';

export default function VendedoresPage() {
  const router = useRouter();
  const [contratos, setContratos] = useState<ContratoVendedor[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  const fetchContratos = useCallback(async () => {
    try {
      const res = await fetch('/api/admin/senales-ip/vendedores');
      if (!res.ok) throw new Error('Error al cargar');
      setContratos(await res.json());
    } catch {
      setError('No se pudieron cargar los contratos de vendedores.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const toggleActiva = async (id: number, activa: boolean) => {
    await fetch(`/api/admin/senales-ip/vendedores/${id}`, {
  ...
  const eliminar = async (id: number) => {
    ...
    await fetch(`/api/admin/senales-ip/vendedores/${id}?hard=true`, { method: 'DELETE' });
    fetchContratos();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Contratos Vendedor</h2>
          <p className="mt-1 text-sm text-muted-foreground">Contratos con terceros externos</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => router.push('/admin/senales-ip')}>← Volver</Button>
          <Button onClick={() => router.push('/admin/senales-ip/vendedores/nueva')}>+ Nuevo contrato</Button>
        </div>
      </div>
                ...
                {contratos.map((c) => (
                  <TableRow key={c.id}>
                    <TableCell>
                      ...
                      {c.mail_comercial_cliente && (
                        <div className="text-xs text-muted-foreground">{c.mail_comercial_cliente}</div>
                      )}
                    </TableCell>
                    ...
                    <TableCell className="text-center">
                      <div className="flex items-center justify-center gap-2">
                        <Switch checked={c.activa} onCheckedChange={() => toggleActiva(c.id, c.activa)} />
                        <span className="text-sm text-muted-foreground">{c.activa ? 'Activo' : 'Inactivo'}</span>
                      </div>
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="sm" onClick={() => router.push(`/admin/senales-ip/vendedores/${c.id}`)}>Ver</Button>
                        <Button variant="ghost" size="sm" onClick={() => router.push(`/admin/senales-ip/vendedores/${c.id}/editar`)}>Editar</Button>
                        <Button variant="ghost" size="sm" onClick={() => eliminar(c.id)}>Eliminar</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
