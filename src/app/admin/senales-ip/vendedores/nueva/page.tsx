...
export default function NuevoContratoVendedorPage() {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    equipos_por_senal: [], condiciones_entrega_equipamiento: '',
  });

  const set = (field: keyof FormData, value: string | EquipoPorSenal[]) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/admin/senales-ip/vendedores');
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">Nuevo Contrato Vendedor</h2>
          <p className="text-sm text-muted-foreground">Contrato con tercero externo</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <Input
                ...
                onChange={(e) => setEquipoInput((p) => ({ ...p, signal: e.target.value }))}
              />
        ...
        <div className="flex gap-2">
          ...
          <Button type="button" variant="outline" onClick={() => router.push('/admin/senales-ip/vendedores')}>Cancelar</Button>
        </div>
      </form>
    </div>
  );
